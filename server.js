...
router.post('/authenticate', function (req, res) {
  ...
  Account.find({
    ...
    if (account.length === 0) {
      ...
      return res.json({
        success: false,
        message: 'Account does not exist based on the email that you provided.'
      });
...
router.route('/reports/:question_id/:sub_index')
  .get(function (req, res, next) {
    ...
    var matchStage = {
      "$match": {
        "assessment.id": +req.params.question_id
      }
    }; //1 would be nutrition
      ...
      [
        ...
        matchStage,
        {
          $project: {
            ...
            subs: {
              "$avg": {
                $slice: ["$assessment.subs", +req.params.sub_index, 1]
              }
            }
          }
        }
...
http.createServer(app).listen(app.get('port'), function () {
  ...
  console.log(process.env.NODE_ENV, ' server listening on port ' + app.get('port'));
});
/**END SERVER*************************************/